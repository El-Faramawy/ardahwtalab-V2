			...
			<table class="table datatable-key-basic">
				...
				<tbody>
					...
					<tr>
						...
						<td><a class="btn btn-primary fa fa-list"
								href="{{route('depts.index',['dept_id' => $info->id])}}"></a></td>
						<td><a class="btn btn-primary fa fa-pencil" href="{{route('depts.edit',$info->id)}}"></a></td>
						<td>{{ $info->home ? 'نعم' : "لا" }}</td>
						<td>{{ $info->slider ? 'نعم' : "لا" }}</td>
						<td>
							<a class="btn btn-danger fa fa-remove" data-table='depts' data-id='{{$info->id}}'></a>
						</td>
					</tr>
					@endforeach
				</tbody>
